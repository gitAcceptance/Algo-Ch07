...
public class MaxHeap {
    ...
    protected void siftup(int pos) {
        ...
        if (pos > 0)
            parent = parent(pos);
        while (pos > 0 && this.heap[pos] > this.heap[parent]) {
            swap(pos, parent);
            pos = parent;
            parent = parent(pos);
        }
...
class MaxHeapOp extends MaxHeap {
    ...
    protected void siftDownRemove(int pos) {
        ...
        // while the parent is smaller than the bottom
        while(this.heap[this.parent(pos)] < this.heap[pos]) {
